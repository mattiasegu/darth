...
from .utils import mask_merge_parallel, xyxy2xywh
...
def det_to_scalabel(dataset, results, out_base, nproc):
    scalabel = []
    ann_id = 0
    print(f"\nStart converting to Scalabel detection format")
    ...
    save(out_path, scalabel)


def ins_seg_to_scalabel(dataset, results, out_base, nproc=4):
    scalabel = []
    ...
    if "bbox_results" in results and "segm_results" in results:
        results = [
            [bbox, segm]
            for bbox, segm in zip(results["bbox_results"], results["segm_results"])
        ]

    track_dicts = []
    img_names = [dataset.data_infos[idx]["file_name"] for idx in range(len(results))]

    print(f"\nStart converting to Scalabel instance segmentation format")
    ann_id = 0
...
def box_track_to_scalabel(dataset, results, out_base, nproc):
    scalabel = []
    track_base = osp.join(out_base, "box_track")
    if not osp.exists(track_base):
        os.makedirs(track_base)

    print(f"\nStart converting to Scalabel box tracking format")
    for idx, track_dict in tqdm(enumerate(results["track_results"])):
        ...
        frame = Frame(
            name=img_name, video_name=vid_name, frame_index=frame_index, labels=[]
        )
...
def seg_track_to_scalabel(dataset, results, out_base, nproc=4):
    bitmask_base = osp.join(out_base, "seg_track")
    if not osp.exists(bitmask_base):
        os.makedirs(bitmask_base)

    print(f"\nStart converting to Scalabel seg tracking format")
    img_names = [
        dataset.data_infos[idx]["file_name"]
        for idx in range(len(results["track_results"]))
    ]
    mask_merge_parallel(results["track_results"], img_names, bitmask_base, nproc)


def preds2scalabel(dataset, results, tasks, out_base, *args, **kwargs):
    metric2func = dict(
        det=det_to_scalabel,
        ins_seg=ins_seg_to_scalabel,
        box_track=box_track_to_scalabel,
        seg_track=seg_track_to_scalabel,
    )
