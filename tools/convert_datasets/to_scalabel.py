...
from darth.core.to_scalabel import preds2scalabel


def parse_args():
    parser = argparse.ArgumentParser(description='darth test model')
    parser.add_argument('config', help='test config file path')
    parser.add_argument('--res', help='output result file')
    parser.add_argument(
        '--scalabel-dir',
        type=str,
        help='path to the folder that will contain files in scalabel format, '
             'e.g. the BDD100K root directory.'
        )
    parser.add_argument(
        '--coco-file',
        type=str,
...
def main():
    ...
    if cfg.get('USE_MMDET', False):
        ...
        from darth.datasets import build_dataloader
    ...
    if args.scalabel_dir:
        preds2scalabel(
            dataset, results, args.task, out_base=args.scalabel_dir, nproc=args.nproc)
